...
export class UniversityConComponent implements OnInit {
  ...
  makeActive(tab:any) {
    ...
    if (this.url.length > 3) {
      ...
      this.router.navigate(['university/' + this.currentUrl + '/' + tab])
    }
    this.activeTab = tab;
    window.scroll({
      top: 425,
...
// sub Tab End
  ...
  getCoursesFees(url:any) {
    this.api.get(`collegemaster/${url}`).subscribe((res: any) => {
      console.log('res: ', this.universityContents);
      if (res.isError) {
        console.log("Something Went wrong ")
      } else {
        let courceName=[]
  ...
  ngOnInit() {
    ...
    if (this.url.length > 3) {
      if (this.url[this.url.length -1] === 'courses-and-fees') {
        const Undata:any = this.universityContents [this.url[2].replace(/-/g, "")]
        this.universityData = Undata[this.url[2].replace(/-/g, "")][this.url[3].replace(/-/g, "")]
    ...
    const data:any = this.universityContents [this.url[2].replace(/-/g, "")]
    ...
    this.universityData.info.forEach((met:any) => {
      if (met.type === 'meta') {
        met.meta.forEach((metaData:any) => {
